...
import { findOrCreateQuickUser, getQuickAccountDestination } from "@/lib/quick-auth";
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.redirect(new URL(getQuickAccountDestination(user, state.returnTo), request.url), 303);
  } catch (error) {
    console.error("[Apple OAuth callback]", error);
    await reportOperationalError(error, { source: "oauth.apple.callback" });
