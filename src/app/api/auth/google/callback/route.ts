...
import { findOrCreateQuickUser, getQuickAccountDestination, safeReturnTo } from "@/lib/quick-auth";
...
export async function GET(request: NextRequest) {
  ...
  if (!code || !state || !expected || state !== expected) {
    return NextResponse.redirect(new URL("/logowanie?error=oauth_state", request.url));
  }

  const clientId = process.env.GOOGLE_CLIENT_ID;
  const clientSecret = process.env.GOOGLE_CLIENT_SECRET;
  if (!clientId || !clientSecret) {
    return NextResponse.redirect(new URL("/logowanie?error=google_not_configured", request.url));
  }
  ...
  try {
    ...
    if (!profile.sub || !profile.email || profile.email_verified !== true) {
      throw new Error("Google nie potwierdził adresu e-mail.");
    }
    ...
    const response = NextResponse.redirect(new URL(getQuickAccountDestination(user, returnTo), request.url));
    response.cookies.delete("ld_oauth_state");
    response.cookies.delete("ld_oauth_return");
    return response;
  } catch (error) {
    console.error("[Google OAuth callback]", error);
    await reportOperationalError(error, { source: "oauth.google.callback" });
