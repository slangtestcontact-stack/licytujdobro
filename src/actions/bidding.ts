...
import { z } from "zod";
import { db } from "@/db";
import { auctions, bids, listings, auctionExtensions, watchlists, users } from "@/db/schema";
...
export async function placeBidAction(_prev: BidResult, formData: FormData): Promise<BidResult> {
  ...
  if (!isFullyVerified(user)) return { ok: false, error: "Potwierdź adres e-mail, aby licytować." };
  if (!user.isAdultConfirmed || !user.acceptedTermsAt || !user.acceptedPrivacyAt || !user.onboardingCompletedAt) {
    return { ok: false, error: "Dokończ profil i zaakceptuj zasady przed złożeniem oferty." };
  }
  ...
  const currentTermsVersion = "2026-07-v2";
  ...
  if (duplicate) {
    const [duplicateAuction] = await db
      .select({ minBidIncrement: auctions.minBidIncrement })
      .from(auctions)
      .where(eq(auctions.id, auctionId))
      .limit(1);
    return {
      ok: true,
      newPrice: Number(duplicate.amount),
      minNextBid: getMinNextBid(
        Number(duplicate.amount),
        Number(duplicateAuction?.minBidIncrement ?? 0),
      ),
    };
  }
  ...
  try {
    const result = await db.transaction(async (tx) => {
      ...
      return {
        ...
        listingShortCode: listing.shortCode,
    ...
    revalidatePublicContent({ listingId: result.listingId });
    return {
      ok: true,
      newPrice: amount,
      minNextBid: getMinNextBid(amount, result.minBidIncrement),
    };
  } catch (error) {
