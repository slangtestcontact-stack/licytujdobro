...
async function ensureCanList() {
  ...
  if (!isFullyVerified(user)) {
    throw new Error("Potwierdź adres e-mail, aby wystawiać przedmioty.");
  }
  if (!user.isAdultConfirmed || !user.acceptedTermsAt || !user.acceptedPrivacyAt || !user.onboardingCompletedAt) {
    throw new Error("Dokończ profil i zaakceptuj zasady przed wystawieniem przedmiotu.");
  }
...
function isOwnedUpload(url: string, listingId: string): boolean {
  if (url.startsWith(`/uploads/${listingId}/`)) return true;
  ...
  try {
    ...
    const publicUrl = new URL(
      configuredPublicUrl.endsWith("/")
        ? configuredPublicUrl
        : `${configuredPublicUrl}/`,
    );
    const basePath = publicUrl.pathname.replace(/\/+$/, "");
    const expectedPath = `${basePath}/listings/${listingId}/`.replace(
      /\/{2,}/g,
      "/",
    );
    ...
    return (
      ...
      uploadedUrl.pathname.startsWith(expectedPath)
    );
  } catch {
    return false;
  }
...
export async function addPhotoAction(params: {
  ...
  try {
    ...
    revalidatePath("/dodaj-przedmiot");
    return { ok: true, photo: createdPhoto };
  } catch (error) {
    return { ok: false, error: (error as Error).message };
  }
}
...
const auctionParamsSchema = z.object({
  ...
  startPrice: z.coerce.number().finite().positive("Podaj kwotę większą od zera.").max(100000, "Kwota jest zbyt wysoka."),
  minBidIncrement: z.coerce.number().finite().positive("Minimalne przebicie musi być większe od zera.").max(100000, "Minimalne przebicie jest zbyt wysokie."),
  mode: z.enum(["FIXED_DONATION", "AUCTION", "INTEREST_THEN_AUCTION"]).default("FIXED_DONATION"),
  interestDurationHours: z.coerce.number().refine((value) => [24, 48, 72].includes(value), "Wybierz 24, 48 lub 72 godziny."),
  auctionDurationHours: z.coerce.number().refine((value) => [12, 24, 48].includes(value), "Wybierz 12, 24 lub 48 godzin."),
  durationDays: z.coerce.number().refine((value) => [3, 5, 7].includes(value), "Wybierz 3, 5 lub 7 dni."),
  preferredDays: z.string().trim().optional().default("Do ustalenia po zakończeniu"),
  preferredHours: z.string().trim().optional().default("Do ustalenia po zakończeniu"),
...
export async function saveAuctionParamsAction(
  ...
  const data = {
    ...parsed.data,
    mode:
      parsed.data.mode === "INTEREST_THEN_AUCTION"
        ? "FIXED_DONATION"
        : parsed.data.mode,
  };
  let listing;
  ...
  const values = {
    ...
    mode: data.mode,
    interestDurationHours: data.interestDurationHours,
