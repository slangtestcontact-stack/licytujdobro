...
import { BellIcon, CheckIcon, ClockIcon, GavelIcon, PackageIcon, PlusIcon, ShieldIcon } from "@/components/icons";
...
export default async function DashboardPage() {
  ...
  return <main className="page-shell py-10">
    <div className="flex flex-wrap items-end justify-between gap-5 border-b border-slate-200 pb-7">
      <div><p className="text-xs font-bold uppercase tracking-[.14em] text-brand-600">Twoje konto</p><h1 className="mt-2 text-3xl font-bold tracking-[-.03em] text-ink">Dzień dobry, {user.firstName}</h1><p className="mt-2 text-sm text-slate-600">Najważniejsze działania, aukcje i spotkania w jednym miejscu.</p></div>
      <LinkButton href="/dodaj-przedmiot"><PlusIcon size={16}/> Wystaw przedmiot</LinkButton>
    </div>
    ...
    <section className="mt-7 grid grid-cols-2 gap-4 border-b border-slate-200 pb-7 lg:grid-cols-4">
      <Summary icon={PackageIcon} label="Aktywne aukcje" value={activeListings}/><Summary icon={GavelIcon} label="Transakcje w toku" value={attention.length}/><Summary icon={CheckIcon} label="Zakończone" value={completed}/><Summary icon={BellIcon} label="Nieprzeczytane" value={unread}/>
    </section>

    <section className="mt-9">
      <div className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-lg bg-amber-50 text-amber-800"><ClockIcon size={18}/></span><div><h2 className="text-xl font-bold text-ink">Wymaga Twojej uwagi</h2><p className="text-sm text-slate-500">Najpilniejsze czynności są pokazane jako pierwsze.</p></div></div>
      {attention.length?<div className="mt-5 divide-y divide-slate-200 border-y border-slate-200">{attention.map((t)=><div key={t.id} className="grid gap-4 py-5 sm:grid-cols-[1fr_auto] sm:items-center"><div><div className="flex flex-wrap items-center gap-2"><p className="font-semibold text-ink">Transakcja {t.id.slice(0,8)}</p><Badge tone="warning">{humanStatus(t.status)}</Badge></div><p className="mt-1 text-sm text-slate-600">Kwota wpłaty: {formatMoney(t.plannedDonationAmount ?? t.amount)} · zaktualizowano {t.updatedAt.toLocaleString("pl-PL")}</p></div><LinkButton href={`/transakcje/${t.id}`} size="sm">Otwórz centrum przekazania</LinkButton></div>)}</div>:<div className="mt-5"><EmptyState title="Wszystko załatwione" description="Nie masz teraz transakcji wymagających działania."/></div>}
    </section>
    ...
    <section className="mt-11 rounded-xl border border-brand-200 bg-brand-50/60 p-5"><div className="flex gap-3"><ShieldIcon size={20} className="mt-0.5 shrink-0 text-brand-700"/><div><h2 className="font-bold text-ink">Pamiętaj podczas spotkania</h2><p className="mt-1 text-sm leading-6 text-slate-600">Finałem udanej aukcji jest wpłata dla Adasia. Podczas spotkania sprawdź przedmiot, wykonaj wpłatę przez oficjalne narzędzia Siepomaga, a po jej potwierdzeniu odbierz przedmiot.</p></div></div></section>
  </main>;
}
