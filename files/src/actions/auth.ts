...
export async function resendPhoneCodeAction(): Promise<ActionResult & { code?: string }> {
  ...
  await getSmsProvider().send(user.phone, `LicytujDobro: Twój kod weryfikacyjny to ${phoneCode}`);
...
export async function requestPasswordResetAction(_prev: ActionResult, formData: FormData): Promise<ActionResult> {
  ...
  await getEmailProvider().send({ to: email, subject: "Reset hasła LicytujDobro", text: `Otwórz link, aby ustawić nowe hasło: ${resetUrl}` });
